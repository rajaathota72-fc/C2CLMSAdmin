...
if st.sidebar.checkbox("Login"):
    if username == "Admin" and password == "c2camsterdam2022":
        with st.sidebar:
        ...
        if menu == "User portal":
            st.markdown("<h1 style = 'text-align:center;  color:#0ea2bd;'>User Details</h1>",
            ...
            st.markdown("<h1 style = 'text-align:center;  color:#0ea2bd;'>Update the scores</h1>",
                        unsafe_allow_html=True)
            cols_score = st.columns([1, 1, 1, 1])
            score_type = cols_score[1].selectbox("Select the score type",
                                                 ["Animations Reward", "Animations Rating", "AI Reward",
                                                  "AI Rating"])
            if score_type == "Animations Reward":
                score = cols_score[2].number_input("Enter the Reward")
                if cols_score[1].checkbox("update"):
                    col.find_one_and_update({"Name": name},
                                            {"$set": {"AnimationXp": details["AnimationXp"] + score}},
                                            upsert=True)
                    st.success("Score updated successfully")
            if score_type == "Animations Rating":
                score = cols_score[2].number_input("Enter the Rating")
                if cols_score[1].checkbox("update"):
                    col.find_one_and_update({"Name": name}, {"$set": {"AnimationR": details["AnimationR"] + score}},
                                            upsert=True)
                    st.success("Score updated successfully")
            if score_type == "AI Reward":
                score = cols_score[2].number_input("Enter the Reward", key="AI")
                if cols_score[1].checkbox("update"):
                    col.find_one_and_update({"Name": name}, {"$set": {"AIXP": details["AIXP"] + score}},
                                            upsert=True)
                    st.success("Score updated successfully")
        ...
        if menu == "Scoreboard":
            ...
            st.table(df2.style.format(
                {"AnimationXp": "{:.0f}✨", "AnimationR": "{:.0f}💫", "AIXP": "{:.0f}✨", "AIR": "{:.0f}💫"}))
